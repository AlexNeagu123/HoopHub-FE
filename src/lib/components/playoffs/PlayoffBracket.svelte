<script lang="ts">
	...
	function isFirstStage(stage: string) {
		return stage === 'Eastern Conference First Round' || stage === 'Western Conference First Round';
	}
	...
	function getFirstTeam(matchup: PlayoffSeries) {
		if (isFirstStage(matchup.stage)) {
			return matchup.winningTeamRank < matchup.losingTeamRank
				? matchup.winningTeam
				: matchup.losingTeam;
		}
	...
	function getSecondTeam(matchup: PlayoffSeries) {
		...
		return matchup.winningTeam.fullName ===
			getTeamByFirstIndex(matchup.stage, matchup.winningTeam, matchup.losingTeam).fullName
			? matchup.losingTeam
			: matchup.winningTeam;
	}
	...
	function getFirstTeamRank(matchup: PlayoffSeries) {
		if (isFirstStage(matchup.stage)) {
			return matchup.winningTeamRank < matchup.losingTeamRank
				? matchup.winningTeamRank
				: matchup.losingTeamRank;
		}
		return matchup.winningTeam.fullName ===
			getTeamByFirstIndex(matchup.stage, matchup.winningTeam, matchup.losingTeam).fullName
			? matchup.winningTeamRank
	...
	function getFirstTeamWins(matchup: PlayoffSeries) {
		if (isFirstStage(matchup.stage)) {
			return matchup.winningTeamRank < matchup.losingTeamRank
				? matchup.winningTeamWins
				: matchup.losingTeamWins;
		}
		return matchup.winningTeam.fullName ===
			getTeamByFirstIndex(matchup.stage, matchup.winningTeam, matchup.losingTeam).fullName
			? matchup.winningTeamWins
	...
	function getBorderType(matchup: PlayoffSeries) {
		...
		return matchup.stage.indexOf('Eastern') !== -1
			? PlayoffBorderTypes.LEFT
			: PlayoffBorderTypes.RIGHT;
	}
</script>
	...
	{#each playoffRounds as round}
		<BracketLevel>
			{#each playoffBracket.stageGrouping[round] as matchup}
				<BracketMatchup
					firstTeam={getFirstTeam(matchup)}
					secondTeam={getSecondTeam(matchup)}
					firstTeamRank={getFirstTeamRank(matchup)}
					secondTeamRank={getSecondTeamRank(matchup)}
					firstTeamWins={getFirstTeamWins(matchup)}
					secondTeamWins={getSecondTeamWins(matchup)}
