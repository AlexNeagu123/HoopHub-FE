...
<div class="bracket-matchup">
	...
	<!-- svelte-ignore a11y-no-static-element-interactions -->
	<div
		class="bracket-team cursor-pointer hover:bg-secondary-700
    {winningTeamIndex === 0 ? 'font-bold' : 'opacity-70'}
    {winningTeamIndex === 0 && borderType === PlayoffBorderTypes.RIGHT
			? 'border-r-4 border-r-tertiary-900'
			: ''}
    {winningTeamIndex === 0 && borderType === PlayoffBorderTypes.LEFT
			? 'border-l-4 border-l-tertiary-900'
			: ''}"
		on:click={() => onTeamClicked(firstTeam)}
	>
		<div class="bracket-name font-semibold">
			{firstTeamRank}.
			<img class="w-[28%] rounded-full bg-transparent mx-1" alt="AB" src={firstTeam.imageUrl} />
			{firstTeam.fullName.split(' ').pop()}
		</div>
    ...
    {winningTeamIndex === 1 && borderType === PlayoffBorderTypes.RIGHT
			? 'border-r-4 border-r-tertiary-900'
			: ''}
    {winningTeamIndex === 1 && borderType === PlayoffBorderTypes.LEFT
			? 'border-l-4 border-l-tertiary-900'
			: ''}"
	...
	>
		<div class="bracket-name font-semibold">
			{secondTeamRank}.
			<img class="w-[28%] rounded-full bg-transparent mx-1" alt="AB" src={secondTeam.imageUrl} />
			{secondTeam.fullName.split(' ').pop()}
		</div>
