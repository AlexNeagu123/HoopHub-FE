<script lang="ts">
	...
	async function toggleVote(isUpvote: boolean) {
		if ((threadVoteStatus === ThreadVoteStatus.UpVoted && isUpvote) 
...
<div class="flex w-1/5 justify-start">
	<button
		on:click={() => toggleVote(true)}
		class="flex hover:variant-filled-primary items-center rounded-full w-1/2 justify-center"
	>
