<script lang="ts">
    import {AppRoute, refreshPage} from "$lib/constants.js";
    ...
    <div class="card m-10 w-1/2 px-10 py-3 drop-shadow variant-filled-surface">
        <div class="flex justify-between">
            <div class="flex flex-col w-1/2">
                <div class="w-full">
                    <FanRank badge={profilePageInfo.fanInfo.fanBadge}
            ...
            <button class="{isOwnProfile ? 'hover:bg-surface-100 hover:opacity-40' : 'cursor-default'}
        w-[20%] rounded-full bg-surface-500 flex justify-center" on:click={() => {
            if (isOwnProfile)
                triggerFileInput();
        }}>
                <Avatar width="w-[90%]" rounded="rounded-full" background="bg-transparent"
                        src={avatarUrl}/>
            </button>

            {#if isOwnProfile}
                <input type="file" bind:this={fileInput} bind:files={files} accept="image/png" class="hidden"
                       on:change={handleFileSelect}/>
            {/if}
        </div>
        <div class="w-1/2">
            <FanStatsContainer upvotes={profilePageInfo.fanInfo.upVotes}
        ...
        <div class="w-full flex justify-between">
            <div class="mt-4 mb-2">
                <TabGroup border="none" justify="justify-start"
                    ...
                    <TabAnchor href="{AppRoute.PLAYER}" selected={true}>Threads</TabAnchor>
                    <TabAnchor href="{AppRoute.PLAYER}/bio">Comments</TabAnchor>
                    <TabAnchor href="{AppRoute.PLAYER}/bio">Reviews</TabAnchor>
                </TabGroup>
            </div>
            {#if isOwnProfile}
                <div class="mt-4 mb-2">
                    <button type="button"
                            class="btn variant-ghost-primary font-semibold  {!changesMade ? 'text-gray-400 ' +
                         'cursor-not-allowed' : 'text-gray-600'}" on:click={async () => {
                             if(!changesMade)
                                 return;
