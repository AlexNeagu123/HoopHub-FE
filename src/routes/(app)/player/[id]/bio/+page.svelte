<script lang="ts">
	...
	let playerFollows = data.playerFollows;
	...
	const groups: string[][] = [];
	for (
		let season = SeasonConstants.currentSeason;
		season >= player.draftYear;
		season -= PlayerConstants.seasonGroupSize
	) {
		const endSeason = (season + 1).toString();
		const startSeason = Math.max(
			season - PlayerConstants.seasonGroupSize + 1,
			player.draftYear
		).toString();
	...
	$: table = {
		...
		body: tableMapperValues(playerStats, [
			'seasonStr',
			'teamStr',
			'gamesPlayed',
			'pts',
	...
	onMount(() => {
		updatePlayerStats(
			(SeasonConstants.currentSeason - PlayerConstants.seasonGroupSize + 1).toString(),
			(SeasonConstants.currentSeason + 1).toString()
		);
	});
</script>

<PlayerExpanded {player} {playerFollows} pageType="bio">
	<select
	...
	{#if isLoading}
		<LoadingIcon />
	{:else}
		<Table {table} tableType={TableTypes.playerTable} {teamIds} />
	{/if}
</PlayerExpanded>
