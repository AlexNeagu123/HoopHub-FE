<script lang="ts">
    ...
    $: teamOptions = data.teams.filter(team => team.conference === value).map(team => ({
        label: team.fullName,
        value: team.id
    }));

    $: teamsDisplayed = searchedTeam ? [searchedTeam] : data.teams.filter(team => team.conference === value);
    ...
    function multiColumnRight(): void {
        let x = 0;
        if (elemTeams.scrollLeft < elemTeams.scrollWidth - elemTeams.clientWidth - 1) x = elemTeams.scrollLeft + elemTeams.clientWidth;
        elemTeams.scroll(x, 0);
    }
